...
# ══════════════════════════════════════════════════════════
with st.sidebar:
    # ── Theme tokens for sidebar (reuse is_dark from above) ──
    if is_dark:
        SB_CARD_BG  = "rgba(255,255,255,0.05)"
        SB_CARD_BDR = "rgba(255,255,255,0.09)"
        SB_TEXT     = "#e2e2e2"
        SB_MUTED    = "#8a8fa8"
        SB_ACT_BG   = "rgba(76,175,80,0.15)"
        SB_DIVIDER  = "rgba(255,255,255,0.08)"
        SB_SHADOW   = "0 2px 12px rgba(0,0,0,0.3)"
        SB_HEADER   = "#e2e2e2"
    else:
        SB_CARD_BG  = "#ffffff"
        SB_CARD_BDR = "#dde3ed"
        SB_TEXT     = "#1a1a2e"
        SB_MUTED    = "#6b7280"
        SB_ACT_BG   = "rgba(46,125,50,0.08)"
        SB_DIVIDER  = "#e2e8f0"
        SB_SHADOW   = "0 2px 12px rgba(0,0,0,0.06)"
        SB_HEADER   = "#1a1a2e"

    st.markdown(f"""
    <style>
    /* ── Sidebar cards for Home page ── */
    .sb-home-card {{
    ...
    .sb-home-card {{ animation: sbFadeIn 0.4s ease backwards; }}
    .sb-home-card:nth-child(1) {{ animation-delay: 0.05s; }}
    .sb-home-card:nth-child(2) {{ animation-delay: 0.10s; }}
    .sb-home-card:nth-child(3) {{ animation-delay: 0.15s; }}
    .sb-home-card:nth-child(4) {{ animation-delay: 0.20s; }}
    </style>
    """, unsafe_allow_html=True)

    # ── App title card ──
    st.markdown(f"""
    <div class="sb-home-card">
        <div style="display:flex; align-items:center; gap:10px; margin-bottom:6px;">
            <span style="font-size:28px;">📚</span>
            <div>
                <div class="sb-home-title">Rekxare Dami</div>
                <div class="sb-home-sub">{t('app_title')}</div>
            </div>
        </div>
        <div style="font-size:10px; color:{SB_MUTED}; font-weight:600; opacity:0.7;">
            {t('slogan')}
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <div class="sb-home-card">
        <div style="font-size:10px; font-weight:800; letter-spacing:1px; text-transform:uppercase; color:{SB_MUTED}; margin-bottom:10px;">
            📊 {t('sidebar_title')}
        </div>
        <div class="sb-stat-grid">
            ...
            <div class="sb-stat-item">
                <div class="sb-stat-number">{st.session_state.daily_seconds // 60}m</div>
    ...
    st.markdown(f"""
    <div class="sb-home-card">
        <div style="font-size:10px; font-weight:800; letter-spacing:1px; text-transform:uppercase; color:{SB_MUTED}; margin-bottom:6px;">
            ⚙️ {t('settings')}
        </div>
    """, unsafe_allow_html=True)
    ...
    # Audio Test
    if st.button("🔊 " + t("test_audio"), use_container_width=True):
        components.html("""
        <script>
            try {
    ...
    if not st.session_state.confirm_clear:
        ...
        st.markdown(f'<div style="background:rgba(239,83,80,0.08); border:1px solid rgba(239,83,80,0.25); border-radius:10px; padding:10px 12px; text-align:center; font-size:13px; font-weight:700; color:#ef5350; margin-bottom:8px;">⚠️ {t("clear_stats")}?</div>', unsafe_allow_html=True)
